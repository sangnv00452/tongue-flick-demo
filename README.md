...
- **Tracking is always drawn:** face landmarks, the lip outline, the search grid and the verdict. Each sample dot is pink for tongue, white for skin, grey for too dark.
...
2. **Look below it.** Each frame, sample a 6×7 grid on the chin skin just below that expected lip line. The tracked lower-lip landmarks themselves are not trusted here, because the tracker drags them onto a protruding tongue.
   - Opening the mouth moves the grid with the jaw.
   - A tongue resting inside the mouth is above the grid.
   - Only a tongue that sticks out over the lip covers it.
   - Extension = how many rows, counted down from the lip, are mostly tongue-coloured.
3. **Colour.** Pixels are compared as chromaticity against the **cheeks in the same frame**, so warm or dim light shifts both sides and cancels out. Pixels below a darkness floor are ignored, and when too few are usable the cue reports *unknown* rather than guessing.
4. **Counter** (`src/flick-counter.js`, pure and unit-tested):
